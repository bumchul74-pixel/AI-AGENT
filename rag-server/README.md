...
The RAG server can also run as an MCP stdio server while keeping the FastAPI
REST API unchanged.
...
```json
{
  "servers": {
    "rag-server": {
      "command": "d:\\workspace\\AI-AGENT\\rag-server\\.venv\\Scripts\\python.exe",
      "args": [
        "d:\\workspace\\AI-AGENT\\rag-server\\run_mcp_server.py"
      ]
    }
  }
}
